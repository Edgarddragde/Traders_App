...
class TradingApp(EClient, EWrapper):
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        daily_done = (len(self._daily_refs) >= self._expected_daily) if self._expected_daily else True
    ...
    def rank_and_print(self):
        ...
        cush = float(self._acct.get("Cushion", 0.0)) # fraction, e.g. 0.05 = 5%
        
        if not self._acct_ready:
            print(f"[GUARD] account account summary not ready: skipping orders this cycle")
            self._done.set()
            return
        if avail < min_needed or cush < 0.02:
            print(f"[GUARD] insufficient funds or cushion (AvailableFunds={avail:.2f}, Cushion={cush:.2%}; no orders)")
            self._done.set()
            return
        
        for reqId, bars in self._hist_data.items():
            sym = self._symbol_by_req.get(reqId, None)
            # error checking
        ...
        # ---- simple demo rule: buy top candidate if gap > +2% and momentum > 0; short if gap < 2% and momentum < 0
        if rows:
            sym, ogap, cgap, fill, mom, av, px = rows[0]
            if ogap > 0.02 and mom > 0 and fill > -0.3:
                print(f"[SIGNAL] LONG {sym} gap={ogap:+.2%} mom={mom:+.4f} fill={fill:+.2f}")
                if USE_SNAPSHOT:
                    self._request_snapshot_and_place(sym, "BUY", ENTRY_QTY)
                else:
                    entry = self._entry_from_last("BUY", px)
                    if entry:
                        self.place_bracket_rr(sym, "BUY", ENTRY_QTY, entry, risk_pct=0.02, rr=2.0)
            elif ogap < -0.02 and mom < 0 and fill < -0.3:
                print(f"[SIGNAL] SHORT {sym} gap={ogap:+.2%} mom={fill:+.2f}")
                if USE_SNAPSHOT:
                    self._request_snapshot_and_place(sym, "SELL", ENTRY_QTY)
                else:
                    entry = self._entry_from_last("SELL", px)
                    if entry:
                        self.place_bracket_rr(sym, "SELL", ENTRY_QTY, entry, risk_pct=0.02, rr=2.0)
                    else:
                        print(f"[BRACKET] skip {sym}: no entry price")
            else:
                print("[SIGNAL] no trade (rule not met)")
        self._done.set()
